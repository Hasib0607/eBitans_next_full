...
import {
  ...
  const MobileNavThree = () => {
    ...
    const bottomBorderActive = 'h-[4px] rounded-t-lg opacity-100 visible transition-all duration-300 ease-linear';
    ...
    return (
      <>
        ...
        {open && (
          <div onClick={() => setOpen(false)} className='bottom-0 right-0 left-0 fixed top-0 z-[6] bg-black bg-opacity-40 lg:cursor-pointer'></div>
        )}
        <div style={{ backgroundColor: design?.header_color }} className='lg:hidden h-[48px] shadow-lg drop-shadow-xl flex justify-around items-center fixed bottom-4 left-2 right-2 z-[100] rounded-2xl'>
          {['category', 'search', 'home', 'cart', 'user'].map((item) => (
            <div className='group' key={item}>
              <div
                onClick={() => {
                  if (item === 'category') setOpen(!open);
                  if (item === 'search') setSearchshow(!searchshow);
              ...
              >
                {item === 'category' &&
                <div className={`${active === 'search' ? 'text-gray-400' : ''} `}>{gridIcon}</div>
                //  <ViewGridIcon width={25} color={design?.text_color} className={active === 'category' ? 'text-gray-400' : null} />
                 }
                {item === 'search' && <div className={`${active === 'search' ? 'text-gray-400' : ''} `}>{searchIcon}
                    {/* <SearchIcon width={25} color={design?.text_color} /> */}
                    </div>}
                {item === 'home' && <HomeIcon width={25} color={design?.text_color} className={active === 'home' ? 'text-gray-400' : ''} />}
                {item === 'cart' && (
                  <div className='relative'>
                    <ShoppingCartIcon width={25} color={design?.text_color} className={active === 'cart' ? 'text-gray-400' : ''} />
                    {cartList.length > 0 && (
                ...
                {item === 'user' && <UserIcon width={25} color={design?.text_color} className={active === 'user' ? 'text-gray-400' : ''} />}
              </div>
              <div style={{ backgroundColor: design?.text_color }} className={active === item ? bottomBorderActive : bottomBorder}></div>
            </div>
          ))}
        </div>
      ...
      <>
        <div className="w-full flex justify-between py-3 lg:cursor-pointer">
          <Link href={`/category/${item.id}`} passHref>
            <span onClick={() => setOpen(!open)} className="flex-1 flex items-center gap-x-2 text-sm text-gray-900 font-medium w-max">
              <img src={`${iconImg}${item?.icon}`} alt={item.name} className='h-5' />
              <p>{item.name}</p>
            </span>
          </Link>
          {item?.cat && (
            <div className="px-4 h-full">
              {show ? (
                <IoIosArrowUp onClick={() => setShow(!show)} className='text-gray-800' />
              ) : (
                <IoIosArrowDown onClick={() => setShow(!show)} className='text-gray-800' />
              )}
            </div>
          )}
        </div>
            ...
            {item.cat?.map((sub:any) => (
              <div className='py-2' key={sub.id}>
                <Link href={`/category/${sub.id}`} passHref>
                  <span onClick={() => setOpen(!open)} className='flex items-center gap-x-2 pb-2'>
                    <img src={`${iconImg}${sub.icon}`} alt={sub.name} className='h-5' />
