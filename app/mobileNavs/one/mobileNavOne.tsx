...
const MobileNavOne = () => {
    ...
    const classesH3 = 'text-gray-800 text-[14px] font-bold -translate-y-3 transition-all duration-300 ease-in-out';
    const classesH31 = 'text-xs font-semibold';

    return (
        <>
            {open && <div onClick={() => setOpen(false)} className='fixed top-0 bottom-0 left-0 right-0 z-[6] bg-black bg-opacity-40 lg:cursor-pointer'></div>}
            <div className='lg:hidden h-[75px] bg-white shadow-lg drop-shadow-xl border border-t border-gray-100 grid grid-cols-5 items-center fixed bottom-0 left-0 right-0 z-10 py-2'>
                ...
                <div onClick={() => { setActive("category"); setOpen(!open); }} className='rounded-full p-3 text-gray-600 flex flex-col justify-center items-center py-2 group'>
                    <div className={active === 'category' ? classes : ''}>
                        {/* <ViewGridIcon width={20}/> */}
                        <div className="group-hover:text-white lg:cursor-pointer" >{gridOutlineIcon}</div>
                    </div>
                    <h3 className={active === 'category' ? classesH3 : classesH31}>Category</h3>
                </div>

                <div onClick={() => { setActive("search"); setOpen(false); setSearchshow(!searchshow); }} className='rounded-full p-3 text-gray-600 flex flex-col justify-center items-center py-2 group'>
                    <div className={active === 'search' ? classes : ''}>
                        {/* <SearchIcon width={20}/> */}
                        <div className="group-hover:text-white" >{searchIcon}</div>
                    </div>
                    <h3 className={active === 'search' ? classesH3 : classesH31}>Search</h3>
                </div>

                <Link href='/' onClick={() => { setActive("home"); setOpen(false); }} className='rounded-full p-3 text-gray-600 flex flex-col justify-center items-center py-2 group'>
                ...
                <div onClick={() => { setActive("cart"); setOpen(false); setCart(!cart); }} className='rounded-full relative p-3 text-gray-600 flex flex-col justify-center items-center py-2 group'>
                ...
                <Link href='/profile' onClick={() => { setActive("user"); setOpen(false); }} className='rounded-full p-3 text-gray-400 hover:text-gray-900 transition-all duration-300 ease-linear flex flex-col justify-center items-center py-2 group'>
            ...
            <div className={`px-4 z-[20]`}>
                <ul className={`pt-5 top-0 bg-white duration-500 fixed md:w-96 w-64 sm:w-80 overflow-y-auto bottom-0 pb-5 z-[20] lg:cursor-pointer ${open ? "left-0 " : "left-[-140%] "}`}>
                    <div className='pb-7 pt-3 px-6'>
                        ...
                        <div className='flex flex-col gap-3 w-[90%]'>
                            {category?.map((item:any) => (
                                <SingleCat key={item?.id} item={item} open={open} setOpen={setOpen} />
                            ))}
                        </div>
                    </div>
                </ul>
...
const SearchDiv: React.FC<SearchDivProps> = ({ setSearchshow }) => {
    ...
    return (
        <>
            <div onClick={() => setSearchshow(false)} className="fixed top-0 bottom-0 left-0 right-0 text-right"></div>
            <motion.div
            ...
            >
                <input
                    ...
                    placeholder='Search'
                />
                <div className='h-5 w-5 absolute top-4 left-4 font-bold z-10' >{searchIcon}</div>
                    <div onClick={() => { setSearchTxt(''); setSearchshow(false); }} className='h-4 w-4 absolute top-4 right-4 font-bold z-10' >{cancelIcon}</div>
    ...
    return (
        <>
            <div className="w-full flex py-3 lg:cursor-pointer">
                <Link href={'/category/' + item.id} onClick={() => setOpen(!open)} className="flex-1 flex items-center gap-x-2 text-sm text-gray-900 font-medium w-max">
                ...
                {item?.cat && (
                    <div className="px-4 h-full">
                        {show ? (
                            <MinusIcon onClick={() => setShow(!show)} className='h-4 w-4 text-gray-800' />
                        ) : (
                ...
                <div className="ml-8">
                    {item?.cat?.map((sub:any) => (
                        <div className='py-2' key={sub?.id}>
                            <Link href={'/category/' + sub.id} onClick={() => setOpen(!open)} className='flex items-center gap-x-2 pb-2'>
...
const gridOutlineIcon=<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" aria-hidden="true" className="sm:h-5 h-4"><path stroke-linecap="round" stroke-linejoin="round" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
const gridIcon = (
    <svg
