    ...
    return (
        <>
            ...
            <div className='lg:hidden h-[48px] bg-white shadow-lg drop-shadow-xl border border-t border-gray-100 flex justify-around items-center fixed bottom-0 left-0 right-0 z-10'>
                <style>{styles}</style>
                <div className='group'>
                    <div style={{ backgroundColor: design?.header_color }} className={active === 'category' ? topBorderActive : topBorder}></div>
                    <div onClick={() => { setOpen(!open); setActive("category") }} className={'rounded-full px-3 py-2 transition-all duration-300 ease-linear'}>
                        <div className={active === 'category' ? "hoverIcon" : ''} >{gridIcon}</div>
                    </div>
                </div>
                <div className='group'>
                    <div style={{ backgroundColor: design?.header_color }} className={active === 'search' ? topBorderActive : topBorder}></div>
                    <div onClick={() => { setSearchshow(!searchshow); setActive('search'); setOpen(false) }} className={'rounded-full px-3 py-2 transition-all duration-300 ease-linear'}>
                        <div className={active === 'search' ? "hoverIcon" : ''} >{searchIcon}</div>
                    </div>
                </div>
                <div className='group'>
                    <div style={{ backgroundColor: design?.header_color }} className={active === 'home' ? topBorderActive : topBorder}></div>
                    <Link href='/' passHref>
                        <div onClick={() => setActive("home")} className='rounded-full px-3 py-2 transition-all duration-300 ease-linear'>
                            <HomeIcon width={25} className={active === 'home' ? "hoverIcon" : ''} />
                        </div>
                    </Link>
                </div>
                <div className='group'>
                    <div style={{ backgroundColor: design?.header_color }} className={active === 'cart' ? topBorderActive : topBorder}></div>
                    <div onClick={() => { setCart(!cart); setActive("cart"); setOpen(false) }} className={'rounded-full relative px-3 py-2 transition-all duration-300 ease-linear'}>
                        <ShoppingCartIcon width={25} className={active === 'cart' ? "hoverIcon" : ''} />
                        {cartList.length > 0 && <div className='absolute h-4 w-4 rounded-full cart-color flex items-center justify-center top-0 right-2'>
                ...
                <div className='group'>
                    <div style={{ backgroundColor: design?.header_color }} className={active === 'user' ? topBorderActive : topBorder}></div>
                    <Link href='/profile' passHref>
                        <div onClick={() => { setActive("user"); setOpen(false) }} className='rounded-full px-3 py-2 transition-all duration-300 ease-linear'>
                            <UserIcon width={25} className={active === 'user' ? "hoverIcon" : ''} />
                        </div>
                    </Link>
                </div>
            </div>
        ...
        <>
            ...
            >
                ...
                <div onClick={() => {
                    setSearchTxt('');
                    setSearchshow(false);
                }} className='h-4 w-4 absolute top-4 right-4 font-bold z-10' >{cancelIcon}</div>
        ...
        <>
            <div className="w-full flex py-3 lg:cursor-pointer">
                <Link href={`/category/${item.id}`} passHref>
                    <div onClick={() => setOpen(!open)} className="flex-1 flex items-center gap-x-2 text-sm text-gray-900 font-medium w-max">
                    ...
                    {item.cat?.map((sub:any) => (
                        <div key={sub.id} className='py-2'>
                            <Link href={`/category/${sub.id}`} passHref>
                                <div onClick={() => setOpen(!open)} className='flex items-center gap-x-2 pb-2'>
