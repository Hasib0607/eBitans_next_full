...
const ProductCardSeven = ({item}:any) => {
    ...
    `
    let discountPrice1 = getPrice(item?.regular_price, item?.discount_price, item?.discount_type)
    return (
        ...
        <style>{overlyImage}</style>
                ...
                <img src={productImg + item.image[0]} alt="Mountain" className='block object-cover object-center h-auto xl:h-[300px] lg:h-[300px] w-[100%] ml-auto mr-auto  group-hover:scale-105 transition-all duration-300 ease-linear image' />
            </div>
            ...
            <div className=' ml-4 mr-4  my-5 lg:my-0 xl:justify-between md:justify-between lg:justify-between lg:absolute top-5 left-5 duration-1000 lg:opacity-0 lg:group-hover:opacity-100'>
                <div>
                    <p className='text-sm  font font-medium w-[130px]' style={{ height: '30px', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{item?.name.charAt(0).toUpperCase() + item?.name.slice(1)}</p>
                ...
                <div className='flex'>
                    <p className='line-through text-sm'> <Taka />{item.regular_price}</p>
                    <p className='text-sm font-medium'> <Taka />{discountPrice1}</p>
                </div>
            </div>
            </Link>
